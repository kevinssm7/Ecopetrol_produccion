    ...
    public class ActualizarDac
    {
        ...
        public int ActualizarVisitaDispensacionCreacionPM(int? idEvaluacion, int? idPlan)
        {
            try
            {
                using (ECOPETROL_DataContexDataContext db = new ECOPETROL_DataContexDataContext())
                {
                    ver_dispen_evaluacion obj2 = db.ver_dispen_evaluacion.FirstOrDefault(x => x.id_evaluacion == idEvaluacion);
                    obj2.estado = 1;
                    obj2.fechaCreacionPlanMejora = DateTime.Now;
                    obj2.idPlanMejora = idPlan;
        ...
        public int ActualizarDescripcionDetallesEvaluacion(int? idEvaluacion)
        {
            try
            {
                using (ECOPETROL_DataContexDataContext db = new ECOPETROL_DataContexDataContext())
                {
                    db.management_dispensacion_actualizarDetallesEvaluacion(idEvaluacion);
                    return 1;
                }
            }
            catch (Exception ex)
        ...
        public int EliminarCancelarVisitasDispen(ver_puntoDispensacion obj)
        {
            try
            {
                using (ECOPETROL_DataContexDataContext db = new ECOPETROL_DataContexDataContext())
                {
                    ver_puntoDispensacion model = db.ver_puntoDispensacion.Where(l => l.id_cargue_verificacion == obj.id_cargue_verificacion).FirstOrDefault();
                    model.motivo = obj.motivo;
                    model.observacion_cancela_elimina = obj.observacion_cancela_elimina;
