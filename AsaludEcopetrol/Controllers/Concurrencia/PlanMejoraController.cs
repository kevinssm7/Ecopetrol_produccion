...
namespace AsaludEcopetrol.Controllers.Concurrencia
{
    ...
    public class PlanMejoraController : Controller
    {
        ...
        #endregion
        public ActionResult IngresoCaracterización(int? idCronograma, int? idVisitaDis)
        {
            ...
            management_planMejoraDispensacion_evaluacionIdResult datEva = idVisitaDis != null ? BusClass.PlanMejoraEvaluacionDispenId(idVisitaDis) : new management_planMejoraDispensacion_evaluacionIdResult();
            ...
            try
            {
                ...
                if (datEva != null)
                {
                    idRegional = datEva.id_ref_regional;
                    idCiudad = datEva.id_ref_ciudades;
                    idUnis = datEva.id_ref_unis;
                    idPrestador = datEva.idprestador;
                    farmacia = datEva.nombre_farmacia;
                }
            }
            catch (Exception ex)
            {
            ...
            ViewBag.idVisitaDis = idVisitaDis;
        ...
        public JsonResult SaveplanMejora(Models.Concurrencia.PlandeMejora Model)
        {
            ...
            try
            {
                ...
                obj.id_visitas_dispen = Model.idVisitaDis;
        ...
        public JsonResult RemoverOtroProducto(int? ID, int? ID2)
        {
            String mensaje = "";
            Models.Concurrencia.PlandeMejora Model = new Models.Concurrencia.PlandeMejora();
            string result = "";

            Model.EliminarDetallePlan((int)ID, ref MsgRes);
            ...
            if (MsgRes.ResponseType == BussinesEnums.EnumTipoRespuesta.Correcto)
            {
                ...
                lista = Model.Cuentadetallefoco((int)ID2, ref MsgRes);
                int i = 0;
                foreach (var item in lista)
                {
                    i += 1;
                ...
                if (list.Count != 0)
                {
                    ...
                    if (ALERTA != "SI")
                    {
                        try
                        {
                            ...
                            if (MsgRes.ResponseType == BussinesEnums.EnumTipoRespuesta.Correcto)
                            {
                                if (Model.idVisitaDis != null && Model.idVisitaDis != 0)
                                {
                                    conVisita = 1;
                                    var actualizaVisDis = BusClass.ActualizarVisitaDispensacionCreacionPM(Model.idVisitaDis, Model.id_plan_de_mejora);
                                }

                                mensaje = "SE INGRESÓ CORRECTAMENTE.";
                                return Json(new { success = true, message = mensaje, id = Model.id_plan_de_mejora, conVisita = conVisita, idVisita = Model.idVisitaDis }, JsonRequestBehavior.AllowGet);
                            }
                            else
                            {
                                mensaje = "ERROR EL INGRESO DEL DETALLE.";
        ...
        public ActionResult TableroControlPlanMejora(int? idEva, DateTime? fechaIni, DateTime? fechaFin, string regional, string nit)
        {
            Models.Concurrencia.PlandeMejora Model = new Models.Concurrencia.PlandeMejora();
            ...
            try
            {
                Lista = Model.PlanTableroGeneral(idEva, fechaIni, fechaFin, regional, nit);
        ...
        public PartialViewResult MostrarAmpliacionPM(int? idPlan, DateTime? fechaMaxima, int? maximoAmpliaciones, DateTime? fechaInicial)
        {
            ...
            try
            {
                ...
                dato = Model.PlanTableroGeneral(null, null, null, null, null).Where(x => x.id_plan_de_mejora == idPlan).FirstOrDefault(); ;
        ...
        public string GetCascadePrestadores(int? regional, int? ciudad, Models.Concurrencia.PlandeMejora Model)
        {
            ...
            {
                result += "<option value='" + item.id_ref_ips_cuentas + "'>" + item.Nombre + "</option>";
            }
            return result;
        }
        ...
        public JsonResult Get2(int? page, int? limit)
        {
            ...
            Lista = Model.PlanTableroGeneral(null, null, null, null, null);
            Lista = Lista.Where(x => x.id_usuario == SesionVar.IDUser).ToList();
            Lista = Lista.OrderBy(x => x.fecha_respuesta).ToList();
...

        {
            ...
            try
            {
                ...
                if (lista != null)
                {
                    ...
                    Sheet.Cells["C1"].Value = "Id visitas dispensación";
                    Sheet.Cells["D1"].Value = "Regional";
                    Sheet.Cells["E1"].Value = "Unis";
                    Sheet.Cells["F1"].Value = "Localidad";
                    Sheet.Cells["G1"].Value = "Prestador";
                    Sheet.Cells["H1"].Value = "Fecha creación";
                    Sheet.Cells["I1"].Value = "Proceso";
                    Sheet.Cells["J1"].Value = "Auditor";
                    Sheet.Cells["K1"].Value = "Fecha respuesta";
                    ...
                    foreach (management_plan_mejora_tableroResult item in lista)
                    {
                        ...
                        Sheet.Cells[string.Format("C{0}", row)].Value = item.id_visitas_dispen;
                        Sheet.Cells[string.Format("D{0}", row)].Value = item.nombre_regional;
                        Sheet.Cells[string.Format("E{0}", row)].Value = item.nom_unis;
                        Sheet.Cells[string.Format("F{0}", row)].Value = item.nombrelocalidad;
                        Sheet.Cells[string.Format("G{0}", row)].Value = item.nom_prestador;
                        Sheet.Cells[string.Format("H{0}", row)].Value = item.fecha_creacion;
                        Sheet.Cells[string.Format("I{0}", row)].Value = item.nom_proceso;
                        Sheet.Cells[string.Format("J{0}", row)].Value = item.nombre;
                        Sheet.Cells[string.Format("K{0}", row)].Value = item.fecha_respuesta.Value.ToString("dd/MM/yyyy");
                        Sheet.Cells[string.Format("L{0}", row)].Value = item.estadoplan;
        ...
        public ActionResult GuardarNotificacionAdmin(int? idPlan, string correos, string observacion)
        {
            ...
            try
            {
                ...
                this.BusClass.ActualizarEstadoPlanMejora(idPlan, new int?(3));
        ...
        public JsonResult EnviarAbiertosNotifi(int? idPlan)
        {
            var mensaje = "";
            var rta = 0;
            ...
            try
            {
                var actualizar = BusClass.ActualizarEstadoPM(idPlan, 4);
                if (actualizar != 0)
                {
                    mensaje = "PLAN DE MEJORA ENVIADO A ABIERTO NOTIFICADO";
                    rta = 1;
                }
                else
                {
                    mensaje = "ERROR AL ENVIAR PLAN DE MEJORA A ABIERTO NOTIFICADO";
                }
            }
            catch (Exception ex)
            {
                var error = ex.Message;
                mensaje = $"PLAN DE MEJORA ENVIADO A ABIERTO NOTIFICADO: {error}";
