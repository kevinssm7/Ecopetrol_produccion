...
namespace AsaludEcopetrol.Controllers.VerificacionEstablecimiento
{
    ...
    public class VerificacionController : Controller
    {
        ...
        public ActionResult TableroPuntoDispensacion(string regional, DateTime? fechaIni, DateTime? fechaFin, string prestador, int? nitprestador, string msg, int? tipo)
        {
            ...
            List<management_dispensacion_evaluacionRelacionResult> dispensacionEliminadas = new List<management_dispensacion_evaluacionRelacionResult>();
            List<management_dispensacion_evaluacionRelacionResult> dispensacionCanceladas = new List<management_dispensacion_evaluacionRelacionResult>();

            List<management_dispensacion_evaluacionRelacionResult> dispensacionEnPM = new List<management_dispensacion_evaluacionRelacionResult>();
            ...
            try
            {
                ...
                dispensacionPendiente = dispensacionTotal.Where(x => x.id_evaluacion == null && x.cancelada != 1 && x.eliminada != 1).ToList();
                dispensacionEvaluadas = dispensacionTotal.Where(x => x.id_evaluacion != null && x.estado == 1 && x.cancelada != 1 && x.eliminada != 1).ToList();
                dispensacionContruccion = dispensacionTotal.Where(x => x.estado == 2 && x.cancelada != 1 && x.eliminada != 1).ToList();
                dispensacionEnPM = dispensacionTotal.Where(x => x.estado == 3 && x.cancelada != 1 && x.eliminada != 1).ToList();

                dispensacionEliminadas = dispensacionTotal.Where(x => x.eliminada == 1).ToList();
                dispensacionCanceladas = dispensacionTotal.Where(x => x.cancelada == 1).ToList();
            ...
            ViewBag.conteoEliminada = dispensacionEliminadas.Count();
            ViewBag.conteoCancelada = dispensacionCanceladas.Count();
            ViewBag.conteoPM = dispensacionEnPM.Count();
            ...
            ViewBag.listaEliminada = dispensacionEliminadas;
            ViewBag.istaCancelada = dispensacionCanceladas;
            ViewBag.listaPM = dispensacionEnPM;
        ...
        public PartialViewResult _ModalGestion(int idVerificacion)
        {
            ...
            ViewBag.tipos = BusClass.TraerTiposActualizarVisita();

            return PartialView();
        }

        public JsonResult ActualizarVisitaDis(int idVerificacion, int motivo, DateTime? fecha, string auditor, string observacion)
        {
            var mensaje = "";

            try
            {
                var motivoTotal = BusClass.TraerTiposActualizarVisita().Where(x => x.id_ref == motivo).Select(x => x.descripcion).FirstOrDefault();

                ver_puntoDispensacion_actualizaciones novedad = new ver_puntoDispensacion_actualizaciones()
                {
                    id_cargue_verificacion = idVerificacion,
                    tipo_novedad = motivo,
                    observacion_motivo = observacion,
                    auditor = auditor,
                ...
                var insertarNovedad = BusClass.InsertarActualizacionVisitasDispen(novedad);
                if (insertarNovedad != 0)
                {
                    ver_puntoDispensacion obj = new ver_puntoDispensacion();
                    ...
                    var respuesta = Model.ActualizarPuntoDispensacion(obj, motivo);

                    if (respuesta != 0)
                    {
                        EnviarCorreoFacturaDevuelta(idVerificacion, motivoTotal, observacion, SesionVar.IDUser, ref MsgRes);

                        mensaje = "ACTUALIZACIÓN GENERADA CORRECTAMENTE.";
                        return Json(new { success = true, message = mensaje }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        mensaje = "ERROR AL GENERAR LA ACTUALIZACIÓN.";
        ...
        public PartialViewResult _ModalGestionEliminarCancelar(int idVerificacion)
        {
            ViewBag.idVerificacion = idVerificacion;

            ViewBag.tipos = BusClass.TraerTiposNovedadVisita();

            return PartialView();
        }

        public JsonResult EliminarCancelarVisitaDis(int? idVerificacion, int? motivo, string observacion)
        {
            var mensaje = "";

            try
            {
                var motivoTotal = BusClass.TraerTiposNovedadVisita().Where(x => x.id_ref == motivo).Select(x => x.descripcion).FirstOrDefault();

                ver_puntoDispensacion_novedades novedad = new ver_puntoDispensacion_novedades()
                {
                    id_cargue_verificacion = idVerificacion,
                    tipo_novedad = motivo,
                    observacion_novedad = observacion,
                    fecha_digita = DateTime.Now,
                    usuario_digita = SesionVar.UserName
                };

                var insertarNovedad = BusClass.InsertarNovedadVisitaDispensacion(novedad);
                if (insertarNovedad != 0)
                {
                    ver_puntoDispensacion obj = new ver_puntoDispensacion();

                    obj.id_cargue_verificacion = (int)idVerificacion;
                    obj.motivo = motivoTotal;
                    ...
                    obj.observacion_cancela_elimina = observacion;

                    var respuesta = Model.EliminarCancelarVisitasDispen(obj);

                    if (respuesta != 0)
                    {
                        EnviarCorreoFacturaDevuelta(idVerificacion, motivoTotal, observacion, SesionVar.IDUser, ref MsgRes);

                        mensaje = "NOVEDAD GENERADA CORRECTAMENTE.";
                        return Json(new { success = true, message = mensaje }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        mensaje = "ERROR AL GENERAR LA NOVEDAD.";
                        return Json(new { success = false, message = mensaje }, JsonRequestBehavior.AllowGet);
                    }
                }
        ...
        public ActionResult EvaluacionDispensacion(int? idPunto, int? tipo, int? idEvaluacion)
        {
            ...
            var idEvaluacionAlterno = 0;

            try
            {
                ver_dispen_evaluacion evaluacion = BusClass.TarerDatosEvaluacionId(idPunto);

                if (evaluacion != null)
                {
                    idEvaluacionAlterno = evaluacion.id_evaluacion;
                }


                var idEvaluacionFiltro = idEvaluacion == null ? idEvaluacionAlterno : idEvaluacion;

                List<ref_verificacion_farmaceutico> titulos = Modelo.Get_refVerificacionFarmaceutita();
                management_dispensacion_evaluacionRelacion_totalIdResult TipicaBase = idEvaluacionFiltro != null ? Modelo.getDispensacionEvaluacionTotalId((int)idEvaluacionFiltro).FirstOrDefault() : new management_dispensacion_evaluacionRelacion_totalIdResult();
                List<management_dispensacion_evaluacionRelacion_totalIdResult> dispensacion = idEvaluacionFiltro != null ? Modelo.getDispensacionEvaluacionTotalId((int)idEvaluacionFiltro).ToList() : new List<management_dispensacion_evaluacionRelacion_totalIdResult>();
                ...
                if (idEvaluacionFiltro != null)
                {
                    archivosEva = BusClass.TraerArchivosVisitasDispensacion((int)idEvaluacionFiltro);
                    Model.id_evaluacion = (int)idEvaluacionFiltro;
                }

                ViewBag.existe = dispensacion.Count();
                ViewBag.dispenEvalu = dispensacion;
                ViewBag.id_evaluacion = idEvaluacionFiltro;
            ...
            try
            {
                ...
                if (idEvaluacion != 0)
                {
                    ...
                    if (evaluacionTotal.Count() != 0)
                    {
                        ...
                        if (finalizado != 0)
                        {
                            var actualiza = BusClass.ActualizarDescripcionDetallesEvaluacion(idEvaluacion);
                        }
                    }
                }

                mensaje = envioPlan == 0 ? $"EVALUACIÓN REALIZADA CORRECTAMENTE - ID {idEvaluacion}."
                    : $"EVALUACIÓN REALIZADA CORRECTAMENTE - ID {idEvaluacion}. SE DEBE ENVIAR A PLAN MEJORA POR TOTAL INFERIOR O IGUAL A 79%.";
                return Json(new { success = true, message = mensaje, idevaluacion = idEvaluacion, envioPlan = envioPlan }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception e)
            {
                var mensajeError = e.Message;
                mensaje = "ERROR EN LA REALIZACIÓN DE LA EVALUACIÓN.";
                return Json(new { success = false, message = mensaje, envioPlan = envioPlan }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public void ExcelExportarIndividual(int id)
        {
            try
            {
                ...
                foreach (management_dispensacion_evaluacionRelacion_totalIdResult item in listareporte)
                {
                    ...
                    Sheet.Cells[string.Format("AD{0}", row)].Value = item.fecha_verificacionOvisita.Value.ToString("dd/MM/yyyy");
                    ...
                    Sheet.Cells[string.Format("AH{0}", row)].Value = Convert.ToDateTime(item.fecha_nuevavisita).ToString("dd/MM/yyyy");
        ...
        public void ExcelExportarTotalHallazgo()
        {
            try
            {
                ...
                Sheet.Cells["A1:AE1"].Style.Font.Bold = true;
                Color colFromHex = Color.FromArgb(22, 54, 92);
                Sheet.Cells["A1:AE1"].Style.Fill.PatternType = ExcelFillStyle.Solid;
                Sheet.Cells["A1:AE1"].Style.Fill.BackgroundColor.SetColor(colFromHex);
                Sheet.Cells["A1:AE1"].Style.Font.Color.SetColor(Color.White);
                Sheet.Cells["A1:AE1"].Style.Font.Name = "Century Gothic";
                ...
                Sheet.Cells["AD1"].Value = "ID DISPENSACIÓN";
                Sheet.Cells["AE1"].Value = "ID EVALUACIÓN";
                ...
                foreach (management_dispensacion_evaluacionRelacion_total_hallazgoResult item in listareporte)
                {
                    ...
                    Sheet.Cells[string.Format("AD{0}", row)].Value = item.id_dispensacion;
                    Sheet.Cells[string.Format("AE{0}", row)].Value = item.id_evaluacion;
                ...
                Sheet.Cells["A" + row + ":AE1" + row].Style.Font.Name = "Century Gothic";

                Sheet.Cells["A:AE"].AutoFitColumns();

                Response.Clear();
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                Response.AppendHeader("Content-Disposition", $"attachment; filename=VisitasEvaluadasHallazgos_{DateTime.Now}.xlsx");
                Response.BinaryWrite(Ep.GetAsByteArray());
                Response.End();
            }
            catch (Exception ex)
        ...
        public JsonResult GuardarActaEvaluacion(int? idEvaluacion, HttpPostedFileBase archivoActa)
        {
            var mensaje = "";
            var rta = 0;
            ver_dispen_evaluacion_acta acta = new ver_dispen_evaluacion_acta();

            try
            {
                acta.id_evaluacion = (int)idEvaluacion;

                if (archivoActa != null)
                {
                    var file = archivoActa;
                    if (file.FileName != "")
                    {
                        ...
                        StringBuilder sbRutaDefinitiva;
                        string strRutaDefinitiva = string.Empty;
                        strRutaDefinitiva = ConfigurationManager.AppSettings["rutaArchivosEvaluacionDispen"];
                        sbRutaDefinitiva = new StringBuilder(strRutaDefinitiva);
                        ...
                        {
                            carpeta = "DocumentosActasPruebas";
                        }

                        ruta = Path.Combine(Request.PhysicalApplicationPath, strRutaDefinitiva + "\\" + "idEvaluacion_" + idEvaluacion + "\\" + carpeta);
                        var nombre = $"DocumentoActa_{idEvaluacion}";
                        ...
                        acta.ruta = archivo;
                        acta.nombre_archivo = $"DocumentoActa_{idEvaluacion}.pdf";
                        acta.usuario_digita = SesionVar.UserName;
                        acta.extension = Path.GetExtension(documento.FileName);
                        acta.fecha_digita = DateTime.Now;

                        var inserta = BusClass.InsertarActaEvaluacion(acta);

                        if (inserta != 0)
                        {
                            return Json(new { success = true, rta = 1, mensaje = "Acta cargada correctamente" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { success = false, mensaje = "Ah ocurrido un error al momento de cargar el acta" });
                        }
                    }
                    else
                    {
        ...
        public ActionResult VerActaEvaluacion(int? idEvaluacion)
        {
            ver_dispen_evaluacion_acta dato = new ver_dispen_evaluacion_acta();
            try
            {
                dato = BusClass.ConsultarArchivoActaEvaluacion(idEvaluacion);

                if (dato != null)
                {
                    var obj = dato;
                    string dirpath = Path.Combine(Request.PhysicalApplicationPath, obj.ruta);
                    string filename = obj.nombre_archivo;
                    string extension = "";
        ...
        public JsonResult EliminarActaVisitas(int? idEvaluacion)
        {
            var mensaje = "";
            var rta = 0;
            ver_dispen_evaluacion_acta dato = new ver_dispen_evaluacion_acta();
            ...
            try
            {
                dato = BusClass.ConsultarArchivoActaEvaluacion((int)idEvaluacion);

                if (dato != null)
                {
                    nombreArchivo = dato.nombre_archivo;
                    idArchivo = dato.id_archivo;
                }

                var elimina = BusClass.EliminarArchivoActaEvaluacion(idEvaluacion);
                if (elimina != 0)
                {
                    var log = EliminarArchivoEvaluacion(idEvaluacion, idArchivo, nombreArchivo, 1);
                ...
                {
                    mensaje = "ERROR AL ELIMINAR EL ACTA";
                }
            }
            ...
            {
                var error = ex.Message;
                mensaje = "ERROR AL ELIMINAR EL ACTA: " + error;
            }

            return Json(new { mensaje = mensaje, rta = rta });
        }

        public JsonResult GuardarInstrumentoEvaluacion(int? idEvaluacionInstrumento, HttpPostedFileBase archivoInstrumento)
        {
            var mensaje = "";
            var rta = 0;
            ver_dispen_evaluacion_instrumentoVerificacion instru = new ver_dispen_evaluacion_instrumentoVerificacion();

            try
            {
                instru.id_evaluacion = (int)idEvaluacionInstrumento;

                if (archivoInstrumento != null)
                {
                    var file = archivoInstrumento;
                    if (file.FileName != "")
                    {
                        HttpPostedFileBase documento = file;
                        ...
                        if (ConfigurationManager.AppSettings["BDActiva"].ToString() == "1")
                        {
                            carpeta = "DocumentosInstrumentos";
                        }
                        else if (ConfigurationManager.AppSettings["BDActiva"].ToString() == "2")
                        {
                            carpeta = "DocumentosInstrumentosPruebas";
                        }

                        ruta = Path.Combine(Request.PhysicalApplicationPath, strRutaDefinitiva + "\\" + "idEvaluacion_" + idEvaluacionInstrumento + "\\" + carpeta);
                        var nombre = $"DocumentoInstrumentoVerificacion_{idEvaluacionInstrumento}";
                        archivo = String.Format("{0}\\{1:yyyyMMdd_hhmm}_{2}{3}", ruta,
                        fecha, nombre, Path.GetExtension(file.FileName));
                        ...
                        instru.ruta = archivo;
                        instru.nombre_archivo = $"DocumentoInstrumentoVerificacion_{idEvaluacionInstrumento}.pdf";
                        instru.usuario_digita = SesionVar.UserName;
                        instru.extension = Path.GetExtension(documento.FileName);
                        instru.fecha_digita = DateTime.Now;

                        var inserta = BusClass.InsertarInstrumentoVerificacionEvaluacion(instru);

                        if (inserta != 0)
                        {
                            return Json(new { success = true, rta = 1, mensaje = "Instrumento verificación cargado correctamente" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { success = false, mensaje = "Ah ocurrido un error al momento de cargar el instrumento verificación" });
                        }
                    }
                    else
                    {
                        return Json(new { success = false, mensaje = "Ah ocurrido un error al momento de cargar el instrumento verificación: Archivo sin nombre" }, JsonRequestBehavior.AllowGet);
                    }
                }
                else
                {
                    return Json(new { success = false, mensaje = "Ah ocurrido un error al momento de cargar el instrumento verificación: No se acepta documento" }, JsonRequestBehavior.AllowGet);
                }

            }
            catch (Exception ex)
            {
                return Json(new { success = false, mensaje = "Ah ocurrido un error al momento de cargar el instrumento verificación: " + ex.Message }, JsonRequestBehavior.AllowGet);
            }
        }

        public ActionResult VerInstrumentoEvaluacion(int? idEvaluacion)
        {
            ver_dispen_evaluacion_instrumentoVerificacion dato = new ver_dispen_evaluacion_instrumentoVerificacion();
            try
            {
                dato = BusClass.ConsultarArchivoInsVerifiacionEvaluacionDispen(idEvaluacion);
        ...
        public JsonResult EliminarInstrumentoVerificacionVisitas(int? idEvaluacion)
        {
            var mensaje = "";
            var rta = 0;
            ver_dispen_evaluacion_instrumentoVerificacion dato = new ver_dispen_evaluacion_instrumentoVerificacion();
            ...
            try
            {
                dato = BusClass.ConsultarArchivoInsVerifiacionEvaluacionDispen((int)idEvaluacion);
                ...
                var elimina = BusClass.EliminarArchivoInstrumentoVerificacionEvaluacion(idEvaluacion);
                if (elimina != 0)
                {
                    var log = EliminarArchivoEvaluacion(idEvaluacion, idArchivo, nombreArchivo, 2);

                    mensaje = "INSTRUMENTO VERIFICACIÓN ELIMINADO CORRECTAMENTE";
                    rta = 1;
                }
                else
                {
                    mensaje = "ERROR AL ELIMINAR EL INSTRUMENTO VERIFICACIÓN";
                }
            }
            ...
            {
                var error = ex.Message;
                mensaje = "ERROR AL ELIMINAR EL INSTRUMENTO VERIFICACIÓN: " + error;
            }

            return Json(new { mensaje = mensaje, rta = rta });
        }

        public int EliminarArchivoEvaluacion(int? idEvaluacion, int? idArchivo, string nombreArchivo, int? tipoArchivo)
        {
            log_ver_dispen_evaluacion_eliminarActaInforme log = new log_ver_dispen_evaluacion_eliminarActaInforme()
            {
            ...
            var inserta = BusClass.InsertarLogEliminadoArchivosEvaluacionDispen(log);

            return inserta;

        }

        public void EnviarCorreoFacturaDevuelta(int? idVerificacion, string motivo, string observacionMotivo, int? idUsuario, ref MessageResponseOBJ MsgRes)
        {
            var smtpSection = (SmtpSection)ConfigurationManager.GetSection("system.net/mailSettings/smtp");

            sis_usuario usuario = BusClass.datosUsuarioId((int)idUsuario);
            ...
            try
            {
                ...
                string mailCSS = "";
                string textBody = "";
                textBody += $"El usuario {usuario.nombre} ha creado una novedad:";
                textBody += $"<br/>";
                textBody += $"<br/>";
                textBody += $"Tipo {motivo}";
                textBody += $"<br/>";
                textBody += $"Observación: {observacionMotivo}";
                ...
                if (bdactiva == "1")
                {
                    mailMessage.To.Add("Coordinacionmedicamentos@asalud.co");
                ...
                mailMessage.Subject = "[Novedad visitas dispensación nro " + idVerificacion + "]";
        ...
        public void ExcelExportarTotalHallazgoIdEvalu(int? idEvaluacion)
        {
            try
            {
                List<management_dispensacion_evaluacionRelacion_total_IdEvaluacionResult> listareporte = Model.getDispensacionEvaluacionTotalIdEvaluacion(idEvaluacion);
                listareporte = listareporte.Where(x => x.id_evaluacion != null).ToList();
                listareporte = listareporte.Where(x => x.valor == 0 && x.decision.Contains("si")).ToList();
                ...
                int row = 2;
                foreach (management_dispensacion_evaluacionRelacion_total_IdEvaluacionResult item in listareporte)
                {
                    var cumplimiento = "";
