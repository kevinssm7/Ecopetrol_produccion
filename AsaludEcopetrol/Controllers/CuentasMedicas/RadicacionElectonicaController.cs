...
namespace AsaludEcopetrol.Controllers.CuentasMedicas
{
    ...
    public class RadicacionElectonicaController : Controller
    {
       ...
       string listadoCodPrestador)
        ...
        public void DescargarReporteTigas(int? idAf, DateTime? fechaIni, DateTime? fechaFin, string codPrestador, string numFactura)
        {
            ...
            try
            {
                ...
                listado = BusClass.TraerListadoCUPSActualziarTigas(idAf, fechaIni, fechaFin, codPrestador, numFactura);
            ...
            int idLote, int idTransaccion, int? idUsuario, int? idDetalle, List<fis_rips_cups> listadoCups)
        {
            ...
            try
            {
                ...
                string codigoLimpioceros = "";

                if (codigoLimpio.StartsWith("0"))
                {
                    codigoLimpioceros = codigoLimpio.Substring(1);
                }

                fis_rips_cups cups = listadoCups.FirstOrDefault(x => x.codigo_cups == codigoLimpio || x.codigo_cups == codigoLimpioceros || "0" + x.codigo_cups == codigoLimpioceros || "0" + x.codigo_cups == codigoLimpio);

                if (cups == null)
                {
                    throw new Exception($"CUPS {con.codConsulta} no existe en tabla de CUPS.");
                }
                else
                {
                    con.codConsulta = codigoLimpio;
                }
        ...
        public ActionResult TableroDescargueFacturasSR()
        {
            ViewBag.regional = BusClass.TraerregionalesFis();
        ...
        public void DescargarReporteHESR(DateTime? fechaIni, DateTime? fechaFin, string regional)
        {
            List<management_fis_reporteHes_sinReglasResult> listado = new List<management_fis_reporteHes_sinReglasResult>();

            try
            {
                ExcelPackage Ep = new ExcelPackage();

                listado = BusClass.TraerDatosReporteHESSR(fechaIni, fechaFin, regional);
                if (listado == null || listado.Count() == 0)
                {
                    string rta = "<script LANGUAGE='JavaScript'>" +
                       "window.alert('SIN DATOS PARA MOSTRAR');" +
                ...
                else
                {
                    ...
                    foreach (management_fis_reporteHes_sinReglasResult item in listado)
                    {
                        Sheet.Cells["A" + row + ":EZ" + row].Style.Font.Size = 10;
